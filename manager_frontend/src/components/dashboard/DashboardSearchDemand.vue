...
<template>
  <section class="rounded-xl border border-slate-200 bg-white p-4 dark:border-slate-700 dark:bg-slate-800">
    <div class="flex flex-wrap items-start justify-between gap-3">
      <div><h2 class="font-semibold text-slate-900 dark:text-white">Поисковый спрос</h2><p class="mt-1 text-xs text-[var(--kitlane-muted)] dark:text-slate-400">Запросы, по которым люди находят сайт</p></div>
    ...
    <div class="mt-3 grid gap-2 sm:grid-cols-2">
      <div v-for="provider in sourceProviders" :key="provider.provider" class="rounded-lg bg-slate-50 px-3 py-2 text-xs dark:bg-slate-900/40">
        <div class="flex items-center justify-between gap-2"><span class="font-semibold text-slate-700 dark:text-slate-200">{{ formatSearchDemandProvider(provider.provider) }}</span><span class="text-[var(--kitlane-muted)] dark:text-slate-400">{{ { fresh: 'Данные получены', stale: 'Данные устарели', error: 'Ошибка загрузки', unconfigured: 'Не подключён' }[provider.status] }}</span></div>
        <p v-if="providerUpdatedAt(provider)" class="mt-1 text-slate-400 dark:text-[var(--kitlane-muted)]">Последнее успешное обновление: {{ providerUpdatedAt(provider) }}</p>
      </div>
    </div>

    <div class="mt-4 flex gap-2 overflow-x-auto pb-1" role="tablist" aria-label="Источник поисковых запросов">
      <button v-for="tab in tabs" :key="tab.value" type="button" role="tab" :aria-selected="activeSource === tab.value" :disabled="!sourceAvailable(tab.value)" class="shrink-0 rounded-lg px-3 py-2 text-sm font-semibold transition disabled:cursor-not-allowed disabled:opacity-45" :class="activeSource === tab.value ? 'bg-[var(--kitlane-accent)] text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200 dark:bg-slate-900 dark:text-slate-300 dark:hover:bg-slate-700'" @click="selectSource(tab.value)">{{ tab.label }}</button>
    </div>
    ...
    <div v-if="sortedQueries.length" class="mt-4">
      <div class="mb-3 flex flex-wrap items-center justify-between gap-3">
        <div class="flex items-center gap-1 overflow-x-auto" aria-label="Сортировка запросов">
          <span class="mr-1 shrink-0 text-xs text-[var(--kitlane-muted)] dark:text-slate-400">Сортировать:</span>
          <button v-for="option in sortOptions" :key="option.value" type="button" class="shrink-0 rounded-md px-2 py-1 text-xs font-semibold" :class="sortKey === option.value ? 'bg-[var(--kitlane-accent-soft)] text-[var(--kitlane-accent-text)]' : 'text-[var(--kitlane-muted)] hover:bg-slate-100 dark:text-slate-400 dark:hover:bg-slate-700'" @click="selectSort(option.value)">{{ option.label }} {{ sortIndicator(option.value) }}</button>
        </div>
      ...
      <div class="hidden overflow-x-auto md:block">
        <table class="w-full min-w-[680px] text-left text-sm">
          ...
          <tbody><tr v-for="query in visibleQueries" :key="`${query.provider}:${query.query}`" class="border-b border-slate-100 last:border-0 dark:border-slate-700"><td class="max-w-sm px-3 py-3 font-medium text-slate-900 dark:text-white">{{ query.query }}</td><td class="px-3 py-3 text-[var(--kitlane-muted)] dark:text-slate-400">{{ formatSearchDemandProvider(query.provider) }}</td><td class="px-3 py-3 text-right text-slate-700 dark:text-slate-200">{{ formatMarketingValue(query.clicks) }}</td><td class="px-3 py-3 text-right text-slate-700 dark:text-slate-200">{{ formatMarketingValue(query.impressions) }}</td><td class="px-3 py-3 text-right text-slate-700 dark:text-slate-200">{{ formatMarketingValue(query.ctr, 'percent') }}</td><td class="px-3 py-3 text-right text-slate-700 dark:text-slate-200">{{ position(query.avg_position) }}</td></tr></tbody>
        </table>
      </div>
      <div class="space-y-3 md:hidden"><article v-for="query in visibleQueries" :key="`${query.provider}:${query.query}`" class="rounded-xl bg-slate-50 p-3 dark:bg-slate-900/40"><div class="flex items-start justify-between gap-3"><h3 class="min-w-0 font-semibold text-slate-900 dark:text-white">{{ query.query }}</h3><span class="shrink-0 text-xs text-[var(--kitlane-muted)] dark:text-slate-400">{{ formatSearchDemandProvider(query.provider) }}</span></div><dl class="mt-3 grid grid-cols-2 gap-3 text-sm"><div><dt class="text-xs text-[var(--kitlane-muted)] dark:text-slate-400">Клики</dt><dd class="mt-0.5 font-semibold text-slate-900 dark:text-white">{{ formatMarketingValue(query.clicks) }}</dd></div><div><dt class="text-xs text-[var(--kitlane-muted)] dark:text-slate-400">Показы</dt><dd class="mt-0.5 font-semibold text-slate-900 dark:text-white">{{ formatMarketingValue(query.impressions) }}</dd></div><div><dt class="text-xs text-[var(--kitlane-muted)] dark:text-slate-400">CTR</dt><dd class="mt-0.5 font-semibold text-slate-900 dark:text-white">{{ formatMarketingValue(query.ctr, 'percent') }}</dd></div><div><dt class="text-xs text-[var(--kitlane-muted)] dark:text-slate-400">Средняя позиция</dt><dd class="mt-0.5 font-semibold text-slate-900 dark:text-white">{{ position(query.avg_position) }}</dd></div></dl></article></div>
