...
<template>
  <section class="space-y-4" aria-labelledby="advertising-heading">
    <div class="flex flex-wrap items-start justify-between gap-3">
      <div><h2 id="advertising-heading" class="text-lg font-semibold text-slate-900 dark:text-white">Эффективность рекламы</h2><p class="mt-1 text-sm text-[var(--kitlane-muted)] dark:text-slate-400">Данные рекламных кабинетов; продажи CRM не сопоставляются без атрибуции.</p></div>
      <a v-if="canManageIntegrations" href="/manager/integrations" class="inline-flex items-center gap-1 text-sm font-semibold text-[var(--kitlane-accent-text)] hover:underline"><ExternalLink class="h-4 w-4" aria-hidden="true" />Настроить подключения</a>
    </div>
