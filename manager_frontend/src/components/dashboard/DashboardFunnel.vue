...
const formatConversion = (value: number | null | undefined) => (
  value != null && value > 100 ? 'не сопоставимо' : formatPercent(value)
);
...
<template>
  <section class="rounded-xl border border-slate-200 bg-white p-4 dark:border-slate-700 dark:bg-slate-800">
    ...
    <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">Этапы за месяц; часть сделок могла начаться раньше</p>
    <ol v-if="stages.length" class="mt-4 space-y-2">
      <li v-for="(stage, index) in stages" :key="stage.stage" class="rounded-lg border border-slate-100 px-3 py-2.5 dark:border-slate-700/70">
        ...
        <div class="mt-1 flex flex-wrap gap-x-3 text-xs text-slate-500 dark:text-slate-400"><span v-if="index">Конверсия: {{ formatConversion(stage.conversion_from_previous_pct) }}</span><span>Цикл: {{ formatDays(stage.avg_cycle_days) }}</span></div>
      </li>
    </ol>
