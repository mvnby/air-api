...
<template>
  <section class="space-y-6" aria-labelledby="site-seo-heading">
    <div class="flex flex-wrap items-start justify-between gap-3"><div><h2 id="site-seo-heading" class="text-lg font-semibold text-slate-900 dark:text-white">Сайт и SEO</h2><p class="mt-1 text-sm text-[var(--kitlane-muted)] dark:text-slate-400">Посещаемость сайта и запросы из поисковых систем.</p></div><a v-if="canManageIntegrations" href="/manager/integrations" class="inline-flex items-center gap-1 text-sm font-semibold text-[var(--kitlane-accent-text)] hover:underline"><ExternalLink class="h-4 w-4" aria-hidden="true" />Настроить подключения</a></div>
    <section class="rounded-xl border border-slate-200 bg-white p-4 dark:border-slate-700 dark:bg-slate-800" aria-labelledby="traffic-heading">
      <div class="flex items-start justify-between gap-3"><div class="flex gap-2"><BarChart3 class="mt-0.5 h-5 w-5 text-[var(--kitlane-accent-text)]" aria-hidden="true" /><div><h3 id="traffic-heading" class="font-semibold text-slate-900 dark:text-white">Посещаемость</h3><p class="mt-1 text-xs text-[var(--kitlane-muted)] dark:text-slate-400">Метрика и Google Analytics 4</p></div></div><span class="rounded-full px-2 py-1 text-xs font-semibold" :class="{ 'bg-emerald-50 text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-300': panelStatus === 'fresh', 'bg-amber-50 text-amber-700 dark:bg-amber-950/40 dark:text-amber-300': panelStatus === 'stale', 'bg-rose-50 text-rose-700 dark:bg-rose-950/40 dark:text-rose-300': panelStatus === 'error', 'bg-slate-100 text-slate-600 dark:bg-slate-700 dark:text-slate-300': panelStatus === 'unconfigured' }">{{ trafficStatus.label }}</span></div>
      <div class="mt-4 grid gap-3 lg:grid-cols-2"><article v-for="provider in providers" :key="provider.provider" class="rounded-lg bg-slate-50 p-3 dark:bg-slate-900/40"><div class="flex flex-wrap items-start justify-between gap-2"><div class="min-w-0"><h4 class="text-sm font-semibold text-slate-900 dark:text-white">{{ formatMarketingProvider(provider.provider) }}</h4><p class="mt-1 text-xs leading-5 text-[var(--kitlane-muted)] dark:text-slate-400">{{ providerMessage(provider) }}</p></div><span class="text-xs font-medium text-[var(--kitlane-muted)] dark:text-slate-400">{{ dashboardMarketingStatus(provider).label }}</span></div><dl v-if="availableMetrics(provider).length" class="mt-3 grid grid-cols-2 gap-x-3 gap-y-2 text-sm"><div v-for="([label, value]) in availableMetrics(provider)" :key="label"><dt class="text-xs text-[var(--kitlane-muted)] dark:text-slate-400">{{ label }}</dt><dd class="mt-1 font-semibold text-slate-900 dark:text-white">{{ value }}</dd></div></dl><p v-else-if="provider.status === 'fresh'" class="mt-3 text-sm text-[var(--kitlane-muted)] dark:text-slate-400">За этот период отчёт пуст.</p><p v-if="updatedAt(provider)" class="mt-3 text-xs text-slate-400 dark:text-[var(--kitlane-muted)]">Последнее успешное обновление: {{ updatedAt(provider) }}</p></article></div>
      <div v-if="marketing.sources?.length" class="mt-4 border-t border-slate-100 pt-3 dark:border-slate-700"><p class="text-xs font-semibold uppercase tracking-wide text-slate-400">Основные источники трафика</p><div class="mt-2 space-y-2"><div v-for="source in visibleSources" :key="source.name" class="flex items-center justify-between gap-3 text-sm"><span class="min-w-0 truncate text-slate-600 dark:text-slate-300">{{ source.name }}</span><span class="shrink-0 font-medium text-slate-900 dark:text-white">{{ formatMarketingValue(source.visits) }} · {{ formatMarketingValue(source.share_pct, 'percent') }}</span></div></div><button v-if="marketing.sources.length > 6" type="button" class="mt-3 text-sm font-semibold text-[var(--kitlane-accent-text)] hover:underline" @click="sourcesExpanded = !sourcesExpanded">{{ sourcesExpanded ? 'Свернуть' : `Показать все (${marketing.sources.length})` }}</button></div>
    </section>
    <div class="flex items-center gap-2 text-sm font-semibold text-slate-700 dark:text-slate-200"><Search class="h-5 w-5 text-[var(--kitlane-accent-text)]" aria-hidden="true" />Поисковый спрос</div>
