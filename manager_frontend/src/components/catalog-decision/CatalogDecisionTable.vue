...
const props = defineProps<{ items: CatalogDecisionItem[]; selectedIds: number[]; sort: CatalogDecisionSort; direction: 'asc' | 'desc'; busy?: boolean; existingIds?: number[] }>();
...
const formLabels: Record<string, string> = { wall: 'Настенный', console: 'Консольный', cassette: 'Кассетный', duct: 'Канальный', floor_ceiling: 'Напольно-потолочный', column: 'Колонный' };
const equipmentDetails = (item: CatalogDecisionItem) => [formLabels[item.indoor_form_factor || ''], item.heating_min_c == null ? 'Обогрев: нет данных' : `Обогрев до ${item.heating_min_c} °C`, item.wifi === 'builtin' ? 'Wi-Fi' : item.wifi === 'ready' ? 'Wi-Fi: опция' : ''].filter(Boolean).join(' · ');
