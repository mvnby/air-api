...
const forms = [
  ...
  { value: 'console', label: 'Консольный', icon: 'vertical_align_bottom' },
  { value: 'cassette', label: 'Кассетный', icon: 'grid_4x4' },
  { value: 'duct', label: 'Канальный', icon: 'air' },
  { value: 'floor_ceiling', label: 'Напольно-потолочный', icon: 'view_stream' },
...
<template>
  <section class="space-y-3 rounded-2xl border border-gray-200 bg-white p-3 shadow-sm md:p-4" aria-label="Быстрые фильтры">
    <div class="flex flex-wrap items-center gap-2">
      <label class="relative basis-full sm:basis-72 sm:flex-1"><span class="material-icons-round pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-gray-400">search</span><input :value="modelValue.search ?? ''" class="w-full rounded-xl border border-gray-200 py-2.5 pl-10 pr-3 text-sm outline-none transition focus:border-teal-500 focus:ring-2 focus:ring-teal-100" placeholder="Например, Gree 12" aria-label="Поиск модели, бренда или серии" @input="update({ search: ($event.target as HTMLInputElement).value || undefined })" /></label>
      <label class="inline-flex min-h-10 shrink-0 cursor-pointer items-center gap-2 rounded-xl border border-gray-200 px-3 text-sm text-gray-700"><input :checked="Boolean(modelValue.hasWifi)" type="checkbox" class="h-4 w-4 rounded border-gray-300 text-teal-600 focus:ring-teal-500" @change="update({ hasWifi: ($event.target as HTMLInputElement).checked || undefined })" />Wi‑Fi</label>
      <button type="button" :class="modelValue.isInverter ? 'border-teal-600 bg-teal-50 text-teal-800' : 'border-gray-200 text-gray-700'" class="inline-flex min-h-10 shrink-0 items-center justify-center gap-2 rounded-xl border px-3 text-sm transition" @click="update({ isInverter: modelValue.isInverter ? undefined : true })"><span class="inline-flex h-4 w-7 items-center rounded-full p-0.5" :class="modelValue.isInverter ? 'bg-teal-600' : 'bg-gray-300'"><span class="h-3 w-3 rounded-full bg-white transition" :class="modelValue.isInverter ? 'translate-x-3' : ''" /></span>Инвертор</button>
      <label class="inline-flex min-h-10 shrink-0 cursor-pointer items-center gap-2 rounded-xl border border-gray-200 px-3 text-sm text-gray-700"><input :checked="Boolean(modelValue.includeOrderable)" type="checkbox" class="h-4 w-4 rounded border-gray-300 text-teal-600 focus:ring-teal-500" @change="update({ includeOrderable: ($event.target as HTMLInputElement).checked })" />Включать заказные</label>
    </div>
    <div class="grid gap-4 lg:grid-cols-2">
      <div class="space-y-2">
    <div class="space-y-1.5"><p class="text-xs font-semibold uppercase tracking-wide text-gray-500">Мощность охлаждения · тыс. БТЕ/ч</p><div class="flex flex-wrap gap-1.5"><button v-for="btu in btuClasses" :key="btu" type="button" class="h-10 min-w-10 rounded-full border px-2 text-sm font-semibold transition" :class="(modelValue.coolingBtuClasses ?? []).includes(btu) ? 'border-teal-600 bg-teal-600 text-white' : 'border-gray-200 text-gray-700 hover:border-teal-300'" :aria-pressed="(modelValue.coolingBtuClasses ?? []).includes(btu)" @click="toggleList('coolingBtuClasses', btu)">{{ btu }}</button></div></div>
    <div class="flex flex-wrap items-center gap-2 text-sm text-gray-600">
      <span class="w-full sm:w-auto">Номинальная, кВт</span>
      <input :value="modelValue.coolingMinKw ?? ''" type="number" min="0" step="0.1" placeholder="От" aria-label="Мощность охлаждения от, кВт" class="w-24 rounded-lg border border-gray-200 px-3 py-2" @input="update({ coolingMinKw: ($event.target as HTMLInputElement).value === '' ? undefined : Number(($event.target as HTMLInputElement).value) })" />
      <span aria-hidden="true">—</span>
      <input :value="modelValue.coolingMaxKw ?? ''" type="number" min="0" step="0.1" placeholder="До" aria-label="Мощность охлаждения до, кВт" class="w-24 rounded-lg border border-gray-200 px-3 py-2" @input="update({ coolingMaxKw: ($event.target as HTMLInputElement).value === '' ? undefined : Number(($event.target as HTMLInputElement).value) })" />
    </div>
      </div>
    <div class="space-y-1.5">
      <p class="text-xs font-semibold uppercase tracking-wide text-gray-500">Обогрев в мороз</p>
      <div class="flex flex-wrap gap-1.5">
        <button v-for="temperature in heatingTemperatures" :key="temperature" type="button" class="min-h-10 rounded-xl border px-4 text-sm font-semibold transition" :class="modelValue.heatingMin === temperature ? 'border-teal-600 bg-teal-600 text-white' : 'border-gray-200 text-gray-700'" :aria-pressed="modelValue.heatingMin === temperature" @click="toggleSingle('heatingMin', temperature)">{{ temperature }} °C</button>
      </div>
    ...
    <div class="space-y-1.5">
      <p class="text-xs font-semibold uppercase tracking-wide text-gray-500">Форма блока</p>
      <div class="flex flex-wrap gap-1.5">
        <button v-for="item in forms" :key="item.value" type="button" class="inline-flex min-h-10 items-center gap-1.5 rounded-xl border px-3 text-sm transition" :class="modelValue.indoorFormFactor === item.value ? 'border-teal-600 bg-teal-50 text-teal-800' : 'border-gray-200 text-gray-700'" :aria-pressed="modelValue.indoorFormFactor === item.value" @click="toggleSingle('indoorFormFactor', item.value)"><span aria-hidden="true" class="material-icons-round text-[18px]">{{ item.icon }}</span>{{ item.label }}</button>
      </div>
    </div>
    <div class="space-y-1.5"><p class="text-xs font-semibold uppercase tracking-wide text-gray-500">Бренды</p><div class="flex flex-wrap gap-1.5"><button v-for="brand in brands" :key="brand.id" type="button" class="rounded-full border px-3 py-1.5 text-sm transition" :class="selectedBrandIds.includes(brand.id) ? 'border-teal-600 bg-teal-600 text-white' : 'border-gray-200 text-gray-700 hover:border-teal-300'" @click="toggleBrand(brand.id)">{{ brand.title }}</button></div></div>
    ...
    <div class="flex justify-end"><button v-if="Object.entries(modelValue).some(([key, value]) => key !== 'isPublished' && (Array.isArray(value) ? value.length > 0 : value !== undefined && value !== false && value !== ''))" type="button" class="text-sm text-gray-500 underline underline-offset-2 hover:text-teal-700" @click="emit('reset')">Сбросить фильтры</button></div>
  </section>
</template>
