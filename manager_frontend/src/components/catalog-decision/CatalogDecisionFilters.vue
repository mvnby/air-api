...
<template>
  <section class="space-y-3 rounded-2xl border border-gray-200 bg-white p-3 shadow-sm md:p-4" aria-label="Быстрые фильтры">
    <div class="flex flex-wrap items-center gap-2">
      ...
      <label class="inline-flex min-h-10 shrink-0 cursor-pointer items-center gap-2 rounded-xl border border-gray-200 px-3 text-sm text-gray-700"><input :checked="Boolean(modelValue.hasWifi)" type="checkbox" class="h-4 w-4 rounded border-gray-300 text-brand-600 focus:ring-brand-500" @change="update({ hasWifi: ($event.target as HTMLInputElement).checked || undefined })" />Wi‑Fi</label>
      <button type="button" :class="modelValue.isInverter ? 'border-brand-600 bg-brand-50 text-brand-800' : 'border-gray-200 text-gray-700'" class="inline-flex min-h-10 shrink-0 items-center justify-center gap-2 rounded-xl border px-3 text-sm transition" @click="update({ isInverter: modelValue.isInverter ? undefined : true })"><span class="inline-flex h-4 w-7 items-center rounded-full p-0.5" :class="modelValue.isInverter ? 'bg-brand-600' : 'bg-gray-300'"><span class="h-3 w-3 rounded-full bg-white transition" :class="modelValue.isInverter ? 'translate-x-3' : ''" /></span>Инвертор</button>
      <label class="inline-flex min-h-10 shrink-0 cursor-pointer items-center gap-2 rounded-xl border border-gray-200 px-3 text-sm text-gray-700"><input :checked="Boolean(modelValue.includeOrderable)" type="checkbox" class="h-4 w-4 rounded border-gray-300 text-brand-600 focus:ring-brand-500" @change="update({ includeOrderable: ($event.target as HTMLInputElement).checked })" />Включать заказные</label>
    </div>
    <div class="grid gap-4 lg:grid-cols-2">
      <div class="space-y-2">
    <div class="space-y-1.5"><p class="text-xs font-semibold uppercase tracking-wide text-gray-500">Мощность охлаждения · тыс. БТЕ/ч</p><div class="flex flex-wrap gap-1.5"><button v-for="btu in btuClasses" :key="btu" type="button" class="h-10 min-w-10 rounded-full border px-2 text-sm font-semibold transition" :class="(modelValue.coolingBtuClasses ?? []).includes(btu) ? 'border-brand-600 bg-brand-600 text-white' : 'border-gray-200 text-gray-700 hover:border-brand-300'" :aria-pressed="(modelValue.coolingBtuClasses ?? []).includes(btu)" @click="toggleList('coolingBtuClasses', btu)">{{ btu }}</button></div></div>
    ...
    <div class="flex justify-end"><button v-if="Object.entries(modelValue).some(([key, value]) => key !== 'isPublished' && (Array.isArray(value) ? value.length > 0 : value !== undefined && value !== false && value !== ''))" type="button" class="text-sm text-gray-500 underline underline-offset-2 hover:text-brand-700" @click="emit('reset')">Сбросить фильтры</button></div>
  </section>
</template>
