...
<template>
  <section class="min-h-[520px] bg-white px-4 py-5 dark:bg-slate-900 sm:px-6">
    <div class="flex flex-col gap-4 border-b border-gray-100 pb-5 dark:border-slate-800 sm:flex-row sm:items-center sm:justify-between">
      <div>
        <p class="text-xs font-bold uppercase tracking-[0.16em] text-brand-700 dark:text-brand-300">Медиа товара</p>
      ...
      <button
        type="button"
        class="inline-flex h-10 items-center justify-center gap-2 rounded-lg bg-brand-600 px-4 text-sm font-semibold text-white shadow-sm transition hover:bg-brand-700"
    ...
    <div v-if="product.main_image" class="mt-6 grid gap-5 lg:grid-cols-[minmax(0,1.5fr)_minmax(240px,0.5fr)]">
      ...
      <div class="grid grid-cols-2 gap-3 content-start">
        <button
          ...
          :key="image.id"
          type="button"
          class="aspect-square overflow-hidden rounded-lg border border-gray-200 bg-gray-100 transition hover:border-brand-400 dark:border-slate-700 dark:bg-slate-800"
          @click="emit('open-editor')"
        >
    ...
    <button
      v-else
      type="button"
      class="mt-6 flex min-h-[360px] w-full flex-col items-center justify-center rounded-lg border border-dashed border-gray-300 bg-gray-50 px-6 text-center transition hover:border-brand-400 hover:bg-brand-50/40 dark:border-slate-700 dark:bg-slate-900 dark:hover:border-brand-700"
      @click="emit('open-editor')"
    >
      ...
      <span class="mt-4 inline-flex h-10 items-center gap-2 rounded-lg bg-brand-600 px-4 text-sm font-semibold text-white">
