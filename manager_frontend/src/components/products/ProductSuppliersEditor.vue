...
const candidateStatusClass = (offer: SupplierOfferCandidate): string => ({
  ...
  free: 'bg-brand-50 text-brand-700',
...
<template>
  <section class="space-y-5">
    <header class="border-b border-gray-100 pb-4 dark:border-slate-800">
      <p class="text-xs font-bold uppercase tracking-[0.16em] text-brand-700 dark:text-brand-300">Поставщики</p>
    ...
    <div class="rounded-lg border border-gray-200 bg-gray-50 p-4 dark:border-slate-700 dark:bg-slate-950/30">
      <div class="flex flex-col gap-3 sm:flex-row sm:items-end">
        ...
        <button type="button" class="h-10 rounded-lg bg-brand-600 px-4 text-sm font-semibold text-white hover:bg-brand-700 disabled:opacity-50" :disabled="stockSaving" @click="emit('save-stock')">{{ stockSaving ? 'Применяем…' : 'Применить остаток' }}</button>
      </div>
    </div>

    <section class="rounded-lg border border-brand-200 bg-brand-50/40 p-4 dark:border-brand-900 dark:bg-brand-950/20">
      <div class="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <p class="text-xs font-bold uppercase tracking-[0.16em] text-brand-700 dark:text-brand-300">Добавить позицию прайса</p>
        ...
        <button type="button" class="inline-flex h-10 items-center justify-center gap-2 rounded-lg bg-brand-600 px-4 text-sm font-semibold text-white disabled:opacity-50" :disabled="candidatesLoading || !supplierId" @click="searchCandidates"><Search class="h-4 w-4" />{{ candidatesLoading ? 'Ищем…' : 'Найти позиции' }}</button>
      </div>
      ...
      <div v-if="candidates.length" class="mt-3 space-y-2">
        <article v-for="offer in candidates" :key="offer.offer_id" class="grid gap-3 rounded-lg border border-brand-100 bg-white p-3 text-sm dark:border-brand-900 dark:bg-slate-900 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-center">
          <div class="min-w-0">
