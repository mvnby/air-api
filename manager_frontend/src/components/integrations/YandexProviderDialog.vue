    ...
    <div v-if="open && provider" class="fixed inset-0 z-[120] flex items-center justify-center bg-slate-950/55 p-4" @click.self="$emit('close')">
      <section role="dialog" aria-modal="true" aria-labelledby="yandex-provider-dialog-title" class="w-full max-w-xl overflow-hidden rounded-3xl bg-white shadow-2xl dark:bg-slate-900">
        ...
        <div v-if="mode === 'help'" data-testid="yandex-provider-help" class="space-y-5 px-6 py-6">
          ...
          <div class="rounded-2xl border border-brand-100 bg-brand-50 p-4 text-sm text-brand-900 dark:border-brand-900 dark:bg-brand-950/40 dark:text-brand-100"><div class="flex gap-3"><ShieldCheck class="mt-0.5 h-5 w-5 shrink-0" /><p>Токен хранится зашифрованно и не отображается в CRM.</p></div></div>
        ...
        <form v-else class="space-y-5 px-6 py-6" @submit.prevent="submit">
          <label v-if="isDirect" class="block"><span class="text-sm font-semibold text-slate-800 dark:text-slate-200">Логин клиента <span class="font-normal text-slate-500">(необязательно)</span></span><input v-model="clientLogin" data-testid="direct-client-login" autocomplete="off" class="mt-2 w-full rounded-xl border border-slate-300 px-4 py-3 text-sm outline-none transition focus:border-brand-500 focus:ring-2 focus:ring-brand-100 dark:border-slate-700 dark:bg-slate-950 dark:text-white dark:focus:ring-brand-950" placeholder="Например, master-vozduha"></label>
          <label class="block"><span class="flex items-center justify-between gap-3 text-sm font-semibold text-slate-800 dark:text-slate-200">OAuth-токен<button type="button" class="inline-flex items-center gap-1 text-xs text-brand-700 hover:text-brand-800" @click="$emit('changeMode', 'help')"><HelpCircle class="h-4 w-4" /> Где взять?</button></span><input v-model="oauthToken" data-testid="yandex-provider-oauth-token" type="password" :required="connection?.state !== 'connected'" autocomplete="new-password" class="mt-2 w-full rounded-xl border border-slate-300 px-4 py-3 text-sm outline-none transition focus:border-brand-500 focus:ring-2 focus:ring-brand-100 dark:border-slate-700 dark:bg-slate-950 dark:text-white dark:focus:ring-brand-950" :placeholder="connection?.state === 'connected' ? 'Оставьте пустым, чтобы сохранить текущий токен' : 'Вставьте OAuth-токен'"></label>
          ...
          <div class="flex flex-wrap justify-end gap-3 border-t border-slate-100 pt-5 dark:border-slate-800"><button type="button" class="rounded-xl border border-slate-200 px-4 py-2.5 text-sm font-semibold text-slate-600 hover:bg-slate-50 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-800" @click="$emit('close')">Отмена</button><button data-testid="yandex-provider-save" type="submit" class="inline-flex items-center gap-2 rounded-xl bg-brand-600 px-5 py-2.5 text-sm font-semibold text-white hover:bg-brand-700 disabled:cursor-not-allowed disabled:opacity-60" :disabled="saving"><Loader2 v-if="saving" class="h-4 w-4 animate-spin" />{{ saving ? 'Проверяем...' : 'Проверить и сохранить' }}</button></div>
        </form>
      </section>
    </div>
  </Teleport>
