    ...
    <div v-if="open && provider" class="fixed inset-0 z-[120] flex items-center justify-center bg-slate-950/55 p-4" @click.self="$emit('close')">
      <section role="dialog" aria-modal="true" aria-labelledby="google-provider-dialog-title" class="w-full max-w-xl overflow-hidden rounded-3xl bg-white shadow-2xl dark:bg-slate-900">
        <header class="flex items-start justify-between gap-4 border-b border-slate-100 px-6 py-5 dark:border-slate-800">
          <div>
            <p class="text-xs font-bold uppercase tracking-[0.16em] text-brand-600">{{ providerName }}</p>
        ...
        <div v-if="mode === 'help'" data-testid="google-provider-help" class="space-y-5 px-6 py-6">
          ...
          <div class="flex flex-wrap gap-3">
            ...
            <button type="button" class="rounded-xl bg-brand-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-brand-700" @click="$emit('changeMode', 'configure')">Перейти к подключению</button>
          </div>
        </div>
        ...
        <form v-else class="space-y-5 px-6 py-6" @submit.prevent="submit">
          ...
          <label v-if="isAnalytics" class="block">
            <span class="flex items-center justify-between gap-3 text-sm font-semibold text-slate-800 dark:text-slate-200">Property ID <button type="button" class="inline-flex items-center gap-1 text-xs text-brand-700 hover:text-brand-800" @click="$emit('changeMode', 'help')"><HelpCircle class="h-4 w-4" /> Где найти?</button></span>
            <input v-model="propertyId" data-testid="ga-property-id" required inputmode="numeric" class="mt-2 w-full rounded-xl border border-slate-300 px-4 py-3 text-sm outline-none transition focus:border-brand-500 focus:ring-2 focus:ring-brand-100 dark:border-slate-700 dark:bg-slate-950 dark:text-white dark:focus:ring-brand-950" placeholder="Например, 123456789">
          </label>
          <template v-if="isAds">
            <label class="block">
              <span class="flex items-center justify-between gap-3 text-sm font-semibold text-slate-800 dark:text-slate-200">Customer ID <button type="button" class="inline-flex items-center gap-1 text-xs text-brand-700 hover:text-brand-800" @click="$emit('changeMode', 'help')"><HelpCircle class="h-4 w-4" /> Где найти?</button></span>
              <input v-model="customerId" data-testid="google-ads-customer-id" required inputmode="numeric" class="mt-2 w-full rounded-xl border border-slate-300 px-4 py-3 text-sm outline-none transition focus:border-brand-500 focus:ring-2 focus:ring-brand-100 dark:border-slate-700 dark:bg-slate-950 dark:text-white dark:focus:ring-brand-950" placeholder="123-456-7890">
            </label>
            <label class="block">
              ...
              <input v-model="loginCustomerId" data-testid="google-ads-login-customer-id" inputmode="numeric" class="mt-2 w-full rounded-xl border border-slate-300 px-4 py-3 text-sm outline-none transition focus:border-brand-500 focus:ring-2 focus:ring-brand-100 dark:border-slate-700 dark:bg-slate-950 dark:text-white dark:focus:ring-brand-950" placeholder="Для управляющего аккаунта MCC">
            </label>
          </template>
          <div v-if="isSearchConsole" class="rounded-2xl border border-slate-200 bg-slate-50 p-4 text-sm leading-6 text-slate-700 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200">Домен CRM определит автоматически из подтверждённого ресурса Search Console. <button type="button" class="ml-1 inline-flex items-center gap-1 font-semibold text-brand-700" @click="$emit('changeMode', 'help')"><HelpCircle class="h-4 w-4" /> Как подготовить?</button></div>
          <div class="rounded-2xl border border-brand-100 bg-brand-50 p-4 text-sm text-brand-900 dark:border-brand-900 dark:bg-brand-950/40 dark:text-brand-100"><div class="flex gap-3"><ShieldCheck class="mt-0.5 h-5 w-5 shrink-0" /><p>Подключение использует защищённую авторизацию OAuth. Доступ можно отозвать в настройках Google.</p></div></div>
          ...
          <div class="flex flex-wrap items-center justify-end gap-3 border-t border-slate-100 pt-5 dark:border-slate-800">
            ...
            <button data-testid="google-authorize" type="submit" class="inline-flex items-center gap-2 rounded-xl bg-brand-600 px-5 py-2.5 text-sm font-semibold text-white hover:bg-brand-700 disabled:cursor-not-allowed disabled:opacity-60" :disabled="saving"><Loader2 v-if="saving" class="h-4 w-4 animate-spin" />Продолжить с Google</button>
          </div>
        </form>
      </section>
    </div>
