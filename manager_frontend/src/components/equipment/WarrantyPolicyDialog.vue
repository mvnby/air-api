      ...
      <section ref="dialogRef" class="flex max-h-[94vh] w-full max-w-2xl flex-col overflow-hidden rounded-t-lg border border-slate-200 bg-white shadow-2xl sm:rounded-lg dark:border-slate-700 dark:bg-slate-900" role="dialog" aria-modal="true" aria-labelledby="warranty-policy-dialog-title" tabindex="-1">
        <header class="z-20 flex shrink-0 items-center justify-between gap-3 border-b border-slate-200 bg-white px-4 py-3 dark:border-slate-700 dark:bg-slate-900">
          <div class="flex min-w-0 items-center gap-2">
            <ShieldCheck class="h-5 w-5 shrink-0 text-brand-700 dark:text-brand-300" />
        ...
        <form id="warranty-policy-form" class="min-h-0 flex-1 space-y-4 overflow-y-auto p-4" @submit.prevent="submit">
          ...
          <div class="grid grid-cols-2 gap-2" aria-label="Тип гарантии">
            <button type="button" class="min-h-10 rounded-md border px-3 text-sm font-semibold" :class="form.coverageType === 'supplier' ? 'border-brand-600 bg-brand-50 text-brand-800 dark:bg-brand-950/40 dark:text-brand-200' : 'border-slate-200 text-slate-600 dark:border-slate-700 dark:text-slate-300'" :aria-pressed="form.coverageType === 'supplier'" @click="form.coverageType = 'supplier'">Оборудование</button>
            <button type="button" class="min-h-10 rounded-md border px-3 text-sm font-semibold" :class="form.coverageType === 'mvn_work' ? 'border-brand-600 bg-brand-50 text-brand-800 dark:bg-brand-950/40 dark:text-brand-200' : 'border-slate-200 text-slate-600 dark:border-slate-700 dark:text-slate-300'" :aria-pressed="form.coverageType === 'mvn_work'" @click="form.coverageType = 'mvn_work'">Работы MVN</button>
          </div>
          ...
          <div class="grid gap-3 sm:grid-cols-2">
            ...
            <div v-if="form.scopeType === 'brand' && form.brandId" class="sm:col-span-2">
              <div class="flex gap-2" role="group" aria-label="Охват серий">
                <button type="button" class="flex-1 rounded-md border px-3 py-2 text-sm font-semibold" :aria-pressed="!form.selectedSeriesOnly" :class="!form.selectedSeriesOnly ? 'border-brand-600 bg-brand-50 text-brand-800 dark:bg-brand-950 dark:text-brand-200' : 'border-slate-200 text-slate-600 dark:border-slate-700 dark:text-slate-300'" @click="form.selectedSeriesOnly = false">Все серии</button>
                <button type="button" class="flex-1 rounded-md border px-3 py-2 text-sm font-semibold" :aria-pressed="form.selectedSeriesOnly" :class="form.selectedSeriesOnly ? 'border-brand-600 bg-brand-50 text-brand-800 dark:bg-brand-950 dark:text-brand-200' : 'border-slate-200 text-slate-600 dark:border-slate-700 dark:text-slate-300'" @click="form.selectedSeriesOnly = true">Выбранные серии</button>
              </div>
          ...
          <div class="grid gap-3 sm:grid-cols-2">
            <label class="field-label">Срок гарантии, месяцев<input v-model="form.durationMonths" class="field-input" type="number" min="1" max="240" required /><span v-if="numberOrNull(form.durationMonths)" class="mt-1 block text-xs text-brand-700 dark:text-brand-300" aria-live="polite">{{ warrantyDurationLabel(numberOrNull(form.durationMonths)) }}</span></label>
          ...
          <label class="flex items-center gap-3 rounded-md border border-slate-200 px-3 py-2 text-sm font-semibold text-slate-700 dark:border-slate-700 dark:text-slate-200"><input v-model="form.maintenanceRequired" type="checkbox" class="h-4 w-4 accent-brand-600" /> Для сохранения гарантии обязательно регулярное ТО</label>
          ...
          <label class="flex items-center gap-3 text-sm font-semibold text-slate-700 dark:text-slate-200"><input v-model="form.isActive" type="checkbox" class="h-4 w-4 accent-brand-600" /> Правило активно</label>
