...
<template>
  <section class="business-section" data-testid="supply-warranty-terms-panel">
    ...
    <template v-if="showWarranty">
      <div :class="showSupply ? 'mt-5 border-t border-slate-200 pt-4 dark:border-slate-700' : ''"><h4 class="business-heading">Гарантия</h4><p class="business-help">Укажите согласованный срок отдельно для оборудования и выполненных работ.</p></div>
      <div class="mt-3 grid gap-3 sm:grid-cols-2"><label class="business-field"><span>На оборудование, мес.</span><input :value="terms.goods_warranty_months ?? ''" class="business-input" type="number" min="1" max="240" @input="updateNumber('goods_warranty_months', $event)" /></label><label class="business-field"><span>На работы, мес.</span><input :value="terms.work_warranty_months ?? ''" class="business-input" type="number" min="1" max="240" @input="updateNumber('work_warranty_months', $event)" /></label></div>
