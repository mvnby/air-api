  ...
  <main class="min-h-full bg-slate-50 p-4 dark:bg-slate-950 sm:p-6 lg:p-8">
    <div class="mx-auto max-w-5xl space-y-6">
      <header class="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        ...
        <button v-if="activeTab === 'site' || activeTab === 'services'" type="button" class="rounded-xl bg-brand-600 px-4 py-2.5 text-sm font-semibold text-white disabled:opacity-60" :disabled="saving || !hasChanges" @click="save">{{ saving ? 'Сохраняем…' : 'Сохранить' }}</button>
      </header>
