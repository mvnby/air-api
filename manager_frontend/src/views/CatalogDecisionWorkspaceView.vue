...
const items = ref<CatalogDecisionItem[]>([]); const brands = ref<Array<{ id: number; title: string }>>([]); const series = ref<Array<{ id: number; title: string }>>([]); const filters = ref<Filters>({ isPublished: true }); const sort = ref<CatalogDecisionSort>('title'); const direction = ref<'asc' | 'desc'>('asc'); const page = ref(1); const pages = ref(1); const total = ref(0); const loading = ref(false); const error = ref('');
