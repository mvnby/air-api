"""Backfill legacy Product.main_image files into product media storage.
...
from services.product_main_image_backfill_service import (  # noqa: E402
    ProductMainImageBackfillService,
)
...
async def backfill_product_main_images_storage(
    *,
    ...
    return await ProductMainImageBackfillService.backfill_to_storage(
        session=session,
        storage=storage,
        execute=execute,
        limit=limit,
...
async def run(
    *,
    ...
    async with async_session_maker() as session:
        return await backfill_product_main_images_storage(
            session=session,
            execute=execute,
            provider=provider,
            limit=limit,
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Plan or execute upload of legacy Product.main_image local files "
...
def print_report(report: dict[str, Any]) -> None:
    print("Product Main Image Storage Backfill")
    ...
    if report["items"]:
        ...
        for item in rows[:50]:
            ...
            print(
                "  - "
                f"product#{item['product_id']} main={item['main_image']} "
