...
TCL_SERIES: tuple[SeriesSeed, ...] = (
    ...
    SeriesSeed(
        ...
        match_slugs=("mobilnye-konditsionery-seriia-pslw", "pslw", "portable"),
        description=(
            "Переносная серия TCL для помещений, где нельзя или не хочется устанавливать "
            "сплит-систему. При выборе важно заранее продумать отвод горячего воздуха, "
            "оконный комплект и обслуживание конденсата."
        ),
        features=(
            "Перемещение на роликах",
            "Оконный монтажный комплект",
            "Режим охлаждения",
            "Режим осушения",
            "Таймер 24 часа",
            "Защита от перелива",
    ...
    async with async_session() as session:
        ...
        if updated and execute:
            await CatalogRevisionService.stage_invalidation(
                session,
                reason="tcl_series_content_seed",
