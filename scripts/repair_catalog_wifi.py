"""Plan or execute the ten source-backed MDV Wi-Fi corrections."""
...
from services.catalog_wifi_repair import WIFI_CORRECTIONS, wifi_repair_plan  # noqa: E402
...
async def run(*, execute_plan_digest: str | None) -> dict:
    async with async_session_maker() as session:
        ...
        if execute_plan_digest is not None and is_replica:
            raise RuntimeError("Wi-Fi repair must execute on the PostgreSQL primary")
        query = select(Product).where(Product.id.in_(WIFI_CORRECTIONS)).order_by(Product.id)
