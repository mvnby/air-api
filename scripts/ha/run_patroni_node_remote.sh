...
SHARED_HOST_BELZAKUPKI_GUARD_SOURCE="scripts/shared_host_belzakupki_suspend.sh"
SHARED_HOST_BELZAKUPKI_GUARD_LIFECYCLE_SOURCE="scripts/ha/shared_host_belzakupki_guard_lifecycle.sh"
...
if [[ "${OPERATION}" == "migrate" || "${OPERATION}" == "deploy" ]]; then
  [[ -f "${CANDIDATE_RUNNER_SOURCE}" && -f "${TRANSACTION_SOURCE}" \
    ...
    && -f "${SHARED_HOST_BELZAKUPKI_GUARD_SOURCE}" \
    && ! -L "${SHARED_HOST_BELZAKUPKI_GUARD_SOURCE}" \
    && -f "${SHARED_HOST_BELZAKUPKI_GUARD_LIFECYCLE_SOURCE}" \
    && ! -L "${SHARED_HOST_BELZAKUPKI_GUARD_LIFECYCLE_SOURCE}" \
...
BUNDLE_SOURCES=(
  ...
  "${SHARED_HOST_BELZAKUPKI_GUARD_SOURCE}"
  "${SHARED_HOST_BELZAKUPKI_GUARD_LIFECYCLE_SOURCE}"
...
SHARED_HOST_BELZAKUPKI_GUARD_REMOTE="${REMOTE_BUNDLE_DIR}/shared_host_belzakupki_suspend.sh"
SHARED_HOST_BELZAKUPKI_GUARD_LIFECYCLE_REMOTE="${REMOTE_BUNDLE_DIR}/shared_host_belzakupki_guard_lifecycle.sh"
...
printf '%s\n%s\n' "${GHCR_PAT:-}" "${BOT_VOICE_TRANSCRIPTION_API_KEY}" | ssh "${SSH_OPTS[@]}" "${REMOTE}" "
  ...
  API_SHARED_HOST_BELZAKUPKI_GUARD_SCRIPT=$(quote "${SHARED_HOST_BELZAKUPKI_GUARD_REMOTE}") \
  API_SHARED_HOST_BELZAKUPKI_GUARD_LIFECYCLE=$(quote "${SHARED_HOST_BELZAKUPKI_GUARD_LIFECYCLE_REMOTE}") \
