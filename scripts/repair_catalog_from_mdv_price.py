"""Plan or apply exact catalog corrections confirmed against the Biocond MDV price.

Planning is read-only. Execution requires the fresh plan digest and a PostgreSQL
primary, and should run under the production deploy lock.
"""
...
from services.catalog_price_spec_repair import load_price_repair_manifest, price_spec_repair_plan  # noqa: E402
...
async def run(*, execute_plan_digest: str | None) -> dict:
    ...
    ids = sorted(entry["id"] for entry in (*manifest["indoor_forms"], *manifest["wifi"]))
    async with async_session_maker() as session:
        ...
        if execute_plan_digest is not None and is_replica:
            raise RuntimeError("Catalog price repair must execute on the PostgreSQL primary")
        ...
        if changes:
            await CatalogRevisionService.stage_invalidation(
                session,
                reason="biocond_mdv_price_catalog_repair",
                product_ids=[product.id for product, _, _ in changes],
            )
