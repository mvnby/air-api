...
class PublicCatalogService:
    """Keep canonical MVN compatible and make every other scope strict."""
    ...
    async def get_catalog_page(
        ...
        *,
        ...
        faceted_tag_ids, resolved_brand_slugs = (
            await PublicCatalogService._resolve_filter_ids(
                session,
                tag_slugs,
                brand_slugs,
            )
        ...
        return {
            "items": [PublicCatalogService.project_row(row) for row in rows],
            "meta": {
                "total": total,
                "page": page,
                "limit": limit,
        ...
        *,
        ...
        if await PublicCatalogService.is_canonical_scope(session, tenant_scope):
            ...
            return PublicProductPage(
                product=PublicCatalogService.project_product(product),
                siblings=[
                    PublicCatalogService.project_product(sibling)
    ...
    async def get_vitebsk_featured_products(
        ...
        *,
        ...
        if await PublicCatalogService.is_canonical_scope(session, tenant_scope):
            ...
            return [PublicCatalogService.project_product(item) for item in products]
        rows = await PublicCatalogDAO.get_vitebsk_featured(
            session,
            tenant_scope=tenant_scope,
            limit=limit,
        )
        return [PublicCatalogService.project_row(row) for row in rows]
    ...
    async def get_checkout_prices(
        ...
        *,
        ...
        if await PublicCatalogService.is_canonical_scope(session, tenant_scope):
            return (
                await PublicCatalogDAO.get_shared_prices_by_ids(
                    session,
        ...
        return (
            await PublicCatalogDAO.get_offer_prices_by_ids(
                session,
                tenant_scope=tenant_scope,
                product_ids=product_ids,
            ),
