...
class VerifiedStorefrontOnboardingPlanToken:
    plan_digest: str
    issued_at: int
    nonce: str


class StorefrontOnboardingPlanToken:
    VERSION = 1
    ...
    def issue(
        ...
        *,
        ...
        cls._validate_digest(plan_digest)
        payload = {
            "v": cls.VERSION,
        ...
        encoded = cls._encode_json(payload)
    ...
    def verify(
        ...
        *,
        now: int | None = None,
    ) -> VerifiedStorefrontOnboardingPlanToken:
        ...
        try:
            ...
            raise StorefrontOnboardingBlockedError(
                "Onboarding plan token is malformed"
            ) from exc
        expected = hmac.new(cls._key(), encoded, hashlib.sha256).digest()
        if not hmac.compare_digest(signature, expected):
            raise StorefrontOnboardingBlockedError(
                "Onboarding plan token signature is invalid"
            )
        try:
            payload = json.loads(encoded.decode("utf-8"))
        except (UnicodeError, json.JSONDecodeError) as exc:
            raise StorefrontOnboardingBlockedError(
                "Onboarding plan token payload is invalid"
            ) from exc
        ...
        if payload["v"] != cls.VERSION or isinstance(payload["v"], bool):
            raise StorefrontOnboardingBlockedError(
                "Onboarding plan token version is unsupported"
            )
        issued_at = payload["iat"]
        if isinstance(issued_at, bool) or not isinstance(issued_at, int):
            raise StorefrontOnboardingBlockedError(
                "Onboarding plan token timestamp is invalid"
            )
        ...
        if issued_at > current + cls.FUTURE_SKEW_SECONDS:
            raise StorefrontOnboardingBlockedError(
                "Onboarding plan token was issued in the future"
            )
        if current - issued_at > cls.MAX_AGE_SECONDS:
            raise StorefrontOnboardingBlockedError(
                "Onboarding plan token expired; run a fresh plan"
            )
        return VerifiedStorefrontOnboardingPlanToken(
            plan_digest=digest,
            issued_at=issued_at,
            nonce=nonce,
        )
    ...
    def _validate_digest(value: str) -> None:
        if not _DIGEST_PATTERN.fullmatch(str(value or "")):
            raise ValueError("Plan digest must be a 64-character SHA-256 digest")
    ...
    def _key() -> bytes:
        return hashlib.sha256(
            b"mvn:storefront-onboarding:plan-token:v1\0"
            + settings.SECRET_KEY.encode("utf-8")
        ).digest()


__all__ = [
    "StorefrontOnboardingPlanToken",
    "VerifiedStorefrontOnboardingPlanToken",
]
