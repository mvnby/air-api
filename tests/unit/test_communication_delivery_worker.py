...
@pytest.mark.asyncio
async def test_runtime_safety_check_runs_immediately_before_claim(
    worker_session_factory,
    monkeypatch,
):
    monkeypatch.setattr(settings, "ADMIN_IDS", "", raising=False)
...
@pytest.mark.asyncio
async def test_runtime_safety_check_runs_immediately_before_provider_send(
    worker_session_factory,
    monkeypatch,
):
    monkeypatch.setattr(settings, "ADMIN_IDS", "", raising=False)
    ...
    worker = CommunicationDeliveryWorker(
        session_factory=worker_session_factory,
        provider=provider,
        worker_id="provider-fence-worker",
        lease_seconds=60,
        safety_check=safety_check,
    )
