...
async def test_compute_product_supply_metrics(db):
    product = Product(title="AC", slug="ac-metrics", price=3000, specs={"area_m2": 25})
...
async def test_compute_metrics_fallback_cost_for_zero_qty_offer(db):
    product = Product(title="AC2", slug="ac-metrics-2", price=2470, specs={"area_m2": 25})
...
async def test_compute_metrics_byn_wholesale_and_incoming_status(db):
    product = Product(title="AC3", slug="ac-metrics-3", price=3330, specs={"area_m2": 25})
