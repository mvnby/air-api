...
3. Сделать изменения и локальную проверку:
   ...
   # Backend: пример проверки публичного представления гарантии.
   pytest -q tests/unit/test_public_warranty_projection.py
   # Manager UI: при изменениях приложения выполните сборку и нужные component tests.
   cd manager_frontend && npm run build
   ```

   Выберите существующие тесты по теме правки; гарантия выше приведена как пример.
   Для изменений только Markdown проверьте локальные ссылки, блоки кода,
   `git diff --check` и сохранность перенесённых правил; локальная сборка
   приложения и тесты БД для них не нужны. Обязательные проверки CI сохраняются.
...
Во время ожидания используйте ограниченный по времени watcher для конкретного
PR/run. Возвращайте краткое изменение статуса или итог; подробные логи читайте
при сбое. Не запускайте частые циклы `sleep` + повторное получение тех же статусов
через модель. Это правило не отменяет проверку CI перед merge и smoke-check после
деплоя.
