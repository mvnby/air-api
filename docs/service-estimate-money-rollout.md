...
Before merging, run the normal CI migration test from an empty PostgreSQL test
database. For production, follow [production data operations](production-data-operations.md)
...
cent prices after the migration, while every old API node continues to receive
only whole-ruble service writes. Fractional service writes return a clear
error. Leave the flag off for the whole first rollout and verify both active
and standby app image revisions after the usual health/products/filters smoke
