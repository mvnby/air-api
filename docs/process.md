...
## TODO Backlog (Manager CRM)

1. Усилить валидацию полей "на входе + на бэкенде" для CRM-форм:
   - `email`: проверка формата на frontend и backend (понятные полевые ошибки),
   - `phone`: сохранить текущую маску + backend-валидацию normalized формата,
   - `inn` (УНП): валидировать как белорусский УНП, не принимать другие форматы,
   - `iban`: валидировать структуру/контрольную сумму IBAN, с акцентом на BY-кейсы.
2. Добавить единые сообщения ошибок в manager-формах:
   - полевые ошибки рядом с полем,
   - верхний toast с краткой причиной.
3. Исследовать адресную валидацию (future):
   - интеграция с картографическим/geocoding сервисом,
   - предупреждение менеджеру, если адрес не найден/сомнительный,
   - не блокировать сохранение на первом этапе, только soft-warning.
