...
### Внутристатейные схемы

Схемы — детерминированный слой библиотеки, а не отдельная свободная стилизация для каждой статьи.

- формат: SVG `1200×760`, со встроенными `title`, `desc` и читаемым HTML-alt;
- фон: Warm White, смысловые карточки — white-to-Ice, радиус 28–32 px;
- базовый контур: Mist Gray 3 px; активный путь: Teal/Deep Teal 4–7 px;
- Coral используется только для неисправности или запрета, Amber — для тепла и предупреждения;
- заголовок схемы 34 px, заголовок шага 25 px, пояснение 18–20 px;
- на одной схеме один вывод, максимум четыре шага или две сравниваемые системы;
- один и тот же условный внутренний и наружный блок используется во всех схемах;
- направление процесса показывается одинаковой открытой стрелкой с rounded caps;
- текст не вшивается в генеративный растр: он остаётся редактируемым внутри SVG;
- исходники формируются скриптами `scripts/build_blog_inline_visuals*.py`, публикационные файлы лежат в `/img/blog/v2/inline/`.

Проверка перед публикацией: XML разбирается без ошибок, изображение читается на ширине 360 px, текст не выходит за карточки, а статья содержит точный alt и короткий `figcaption` с выводом.

### Навигационные карточки услуг

Карточка услуги использует тот же понятный язык, что и обложка статьи: реалистичная архитектурная сцена с одним инженерным выводом. Пользователь должен узнать задачу по изображению до чтения описания.

- формат: WebP `1672×941`, соотношение `16:9`; значимые объекты остаются внутри центральных 85%;
- единые камера, свет, материалы и семейство оборудования берутся из master-контракта MVN Climate Atlas;
- один кадр показывает один объектный сценарий — скрытую трассу, готовый монтаж, безопасный демонтаж, глубокую чистку, диагностику, топологию VRF или резерв серверной;
- внутри нет текста, цифр, логотипов и псевдоинтерфейса: название и контекст остаются в HTML;
- бытовой внутренний и наружный блок повторяют `MVN Reference Split 01`;
- три линии AirFlow используются только там, где поток действительно является частью смысла; не добавлять их декоративно;
- AirFlow — ровно три бирюзовые линии; холодильный контур — правдоподобная парная трасса, дренаж — одна линия с видимым уклоном, кабель не подменяет трассу;
- Amber означает подготовку, тепло или резерв, Coral — только локальную неисправность, Teal — нормальный маршрут или действие;
- люди вторичны и показывают конкретную операцию; нейтральная рабочая одежда, без постановочного рекламного портрета;
- картинка внутри кликабельной карточки получает пустой `alt`, если соседний HTML уже полностью называет и объясняет услугу;
- публикационный набор хранится в `/img/services/v3/`, а воспроизводимый prompt-манифест — в `docs/service-card-visual-library.md`;
- SVG-набор `/img/services/v2/` оставлен только для безопасного отката и не используется в интерфейсе.

Перед публикацией набор проверяется целиком: одинаковая перспектива и свет, семь разных сюжетов, технически правдоподобные соединения и читаемость полного кадра на мобильном экране.
...
`/img/blog/v2/{slug}-hero.webp` для обложек и `/img/blog/v2/inline/{diagram-name}.svg` для внутристатейных схем.
