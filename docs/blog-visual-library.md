...
Карточка услуги — не уменьшенная обложка статьи, а быстрый визуальный указатель. Она должна читаться до текста и отличать одну задачу от другой на ширине 124–360 px.

- формат: SVG `1200×760`, единый светлый артборд с радиусом 48 px;
- внутри нет текста, цифр, логотипов и псевдоинтерфейса: название и контекст остаются в HTML;
- один кадр показывает один объектный сценарий — трассу, пару блоков, узел чистки, неисправность, демонтаж, топологию VRF или резерв серверной;
- постоянные элементы: угловые инженерные скобы, точка замера, Warm White/Ice фон, Deep Teal контур и один смысловой акцент;
...
- AirFlow — ровно три бирюзовые кривые с одинаковыми открытыми наконечниками; холодильный контур — парная линия без стрелок, дренаж — одна линия с видимым уклоном, кабель — Amber;
- Amber означает подготовку или внимание, Coral — только неисправность, Teal — нормальный маршрут или действие;
- картинка внутри кликабельной карточки получает пустой `alt`, если соседний HTML уже полностью называет и объясняет услугу;
- исходники формируются `scripts/build_service_card_visuals.py`, публикационный набор хранится в `/img/services/v2/`.

Перед публикацией набор проверяется целиком: одинаковая рамка и масштаб, семь разных силуэтов, технически правдоподобные соединения и читаемость мобильной версии без увеличения изображения.
