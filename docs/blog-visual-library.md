...
4. Генерировать остальные 9 hero по одному, не пакетной случайной серией.
5. Удалять весь сгенерированный текст; необходимые числа и подписи добавлять отдельным SVG/HTML-слоем.
6. Проверять thumbnail на ширине 360 px: тема должна считываться без заголовка.
7. Экспортировать AVIF + WebP; PNG хранить только как production master.
8. После замены удалить неиспользуемые legacy-файлы и проверить `npm run audit:theme` и `npm run build`.
